...
include("server connect.php");
 ...
 if(isset($_POST['b1']))
 {
    ...
    if($f>0)
    {
        ...
        if($f7>=1 || $f7==0)
        {
        $sql2=" create table user$no ( name varchar(30), number bigint(11),primary key(number) )"; //query 
        
        $result13=mysqli_query($con,$sql2);
         $f5=mysqli_affected_rows($con);
         $sql5="create index number on user$no (number)";
         $result17=mysqli_query($con,$sql5);
  ...
  if(isset($_POST['b3']) )
	   {
        ...
        {
			$sqlchck7="select * from signup where mobile=$no1 ";
		...
		if( $f1>0 )
		{
			header("location:http://deb200.rf.gd/index.php?name=".$user1."&mobile=".$no1."");
	    }
	}
	}
